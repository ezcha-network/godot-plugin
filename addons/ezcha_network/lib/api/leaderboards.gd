...
var _update_mode_map: PoolStringArray = PoolStringArray(["set", "add", "subtract"])
...
## Mode should be EzchaLeaderboardAPI.UpdateMode
func post_entry_client(leaderboard_id: String, session_token: String, score: float, mode: int = UpdateMode.SET) -> EzchaLeaderboardQueuedResponse:
...
## Mode should be EzchaLeaderboardAPI.UpdateMode
func post_entry_server(leaderboard_id: String, user_id: String, score: float, mode: int = UpdateMode.SET) -> EzchaLeaderboardQueuedResponse:
	var resp: EzchaLeaderboardQueuedResponse = EzchaLeaderboardQueuedResponse.new()
	EzchaRequestBuilder.new()\
		.set_method(HTTPClient.METHOD_POST)\
